<div align="center">

# Hi, I'm ChiehYao Hsu 👋

<img src="https://readme-typing-svg.demolab.com?font=Inter&weight=500&size=22&duration=3500&pause=900&center=true&vCenter=true&width=900&lines=Happy+to+chat+about+agent+systems%2C+LLM%2C+and+AI+application" />

</div>

---

I’m Joel, an **LLM Application Engineer** who enjoys turning AI capabilities into real, usable systems.

My recent work focuses on **agent workflows**, **tool-using LLM applications**, **workflow automation**, **file processing**, and **secure deployment patterns**. I’m especially interested in making AI systems not only powerful, but also practical, auditable, and production-friendly.

* 🌱 Currently building around **AI agents**, **MCP-style tool orchestration**, and **LLM workflows**
* 🛠️ Interested in **automation**, **developer tooling**, **file processing**, and **cloud-based AI systems**
* 🛡️ Exploring how to make AI applications more **secure**, **structured**, and **reliable**
* 🤝 Open to collaborating on **LLM application** and **AI engineering** projects

---

## Published Packages

A few small tools I’ve published while exploring **agent scaffolding**, **MCP server development**, **file conversion**, and **AI-assisted development workflows**.

| Package                                                              | Ecosystem | Contribution Area                                | Purpose                                                                                                                                     |
| -------------------------------------------------------------------- | --------- | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------- |
| [`convilyn`](https://pypi.org/project/convilyn/)                     | PyPI      | Python SDK / **File Conversion** / **Agentic Workflows** | An official Python SDK for file conversion and agentic workflows, designed to support AI-powered document and file processing applications. |
| [`langgraph-forge`](https://pypi.org/project/langgraph-forge/)       | PyPI      | LangGraph agent scaffolding                      | A starter toolkit for setting up runnable LangGraph agent projects with common providers, patterns, and deployment options.                 |
| [`mcp-forge-cli`](https://pypi.org/project/mcp-forge-cli/)           | PyPI      | MCP project generator                            | A CLI for bootstrapping structured MCP server projects without hand-creating the same project layout each time.                             |
| [`mcp-forge-core`](https://pypi.org/project/mcp-forge-core/)         | PyPI      | MCP server framework                             | A lightweight core layer for building MCP servers with cleaner interfaces and swappable provider implementations.                           |
| [`@hey-echodev/cli`](https://www.npmjs.com/package/@hey-echodev/cli) | npm       | AI / developer workflow CLI                      | A terminal-first tool for keeping useful project context around AI-assisted development workflows.                                          |
| [`redactkit`](https://pypi.org/project/redactkit/)                   | PyPI      | Python / Development Toolkit                     | Production-hardened Python redaction for structured logs, LLM agent payloads, and AWS-signed URLs. Zero deps.                               |

---

## Selected Work

| Project                                                              | Recognition                      | What it does                                                                                  |
| -------------------------------------------------------------------- | -------------------------------- | --------------------------------------------------------------------------------------------- |
| [`AI-Filing-System`](https://github.com/leoliu5550/AI-Filing-System) | 2025 Qualcomm Hackathon Champion | An AI-powered file organization system that analyzes and categorizes documents automatically. |

---

## Technologies I Use

![Python](https://img.shields.io/badge/-Python-3776AB?style=flat\&logo=python\&logoColor=white)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat\&logo=typescript\&logoColor=white)
![FastAPI](https://img.shields.io/badge/-FastAPI-009688?style=flat\&logo=fastapi\&logoColor=white)
![Docker](https://img.shields.io/badge/-Docker-2496ED?style=flat\&logo=docker\&logoColor=white)
![AWS](https://img.shields.io/badge/-AWS-232F3E?style=flat\&logo=amazon-aws\&logoColor=white)
![GCP](https://img.shields.io/badge/-GCP-4285F4?style=flat\&logo=google-cloud\&logoColor=white)
![Amazon Bedrock](https://img.shields.io/badge/-Amazon%20Bedrock-FF9900?style=flat\&logo=amazonaws\&logoColor=white)

![LLM Applications](https://img.shields.io/badge/-LLM%20Applications-000000?style=flat)
![AI Agents](https://img.shields.io/badge/-AI%20Agents-111111?style=flat)
![Tool Orchestration](https://img.shields.io/badge/-Tool%20Orchestration-333333?style=flat)
![Workflow Automation](https://img.shields.io/badge/-Workflow%20Automation-555555?style=flat)
![File Processing](https://img.shields.io/badge/-File%20Processing-666666?style=flat)
